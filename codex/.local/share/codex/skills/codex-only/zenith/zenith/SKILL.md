...
- This workspace is initialized for Codex through `.codex/config.toml`. The host should
...
- If the MCP server is unavailable, verify the installation from the Zenith
  ...
  `uv run zenith init --workspace-dir <workspace> --agent codex`.
- The underlying server can be started from the Zenith checkout with
  ...
  to its MCP client; do not start a second copy when `.codex/config.toml` is already
  connected.
- Zenith workers require the relevant ACP adapter. For Codex, verify
  `command -v codex-acp`; install it with
  `npm install -g @agentclientprotocol/codex-acp` when missing.
