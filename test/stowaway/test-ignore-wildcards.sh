...
# Test .stowaway-ignore: wildcard pattern exclusion

SCRIPT_DIR="$(dirname "$0")"
source "$SCRIPT_DIR/test-lib.sh"

FIXTURE_DIR="$SCRIPT_DIR/fixtures/scenarios/ignore-feature/wildcard-exclude"
TEST_DIR="/tmp/stowaway-test-ignore-wildcards-$$"

echo "🧪 Testing .stowaway-ignore: wildcard pattern exclusion..."
...
# Verify prod-pkg was processed
verify_directory_processed "$TEST_DIR" "prod-pkg" "prod-pkg should be processed" || {
	cleanup_test_env "$TEST_DIR"
	exit 1
}

# Verify test-pkg was EXCLUDED (matches test*)
verify_directory_excluded "$TEST_DIR" "test-pkg" "test-pkg should be excluded" || {
	cleanup_test_env "$TEST_DIR"
	exit 1
}

# Verify test-other was EXCLUDED (matches test*)
verify_directory_excluded "$TEST_DIR" "test-other" "test-other should be excluded" || {
	cleanup_test_env "$TEST_DIR"
	exit 1
}
...
echo "🎉 Wildcard ignore test completed successfully!"
