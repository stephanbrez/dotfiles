...
# Test .stowaway-ignore: comment and whitespace handling

SCRIPT_DIR="$(dirname "$0")"
source "$SCRIPT_DIR/test-lib.sh"

FIXTURE_DIR="$SCRIPT_DIR/fixtures/scenarios/ignore-feature/comments-exclude"
TEST_DIR="/tmp/stowaway-test-ignore-comments-$$"

echo "🧪 Testing .stowaway-ignore: comment and whitespace handling..."

# Setup test environment
setup_test_env "$FIXTURE_DIR" "$TEST_DIR"

# Verify .stowaway-ignore file exists
verify_ignore_file_exists "$TEST_DIR" "Ignore file with comments exists" || {
...
# Verify installers was EXCLUDED (ignoring comments)
verify_directory_excluded "$TEST_DIR" "installers" "installers should be excluded" || {
	cleanup_test_env "$TEST_DIR"
	exit 1
}

# Verify test-pkg was EXCLUDED (ignoring comments)
...
if [ "$PACKAGE_COUNT" -eq 2 ]; then
	echo "✅ Exactly 2 packages processed (comments ignored)"
else
...
echo "🎉 Comment handling test completed successfully!"
