...
# Test .stowaway-ignore: behavior with empty ignore file

SCRIPT_DIR="$(dirname "$0")"
source "$SCRIPT_DIR/test-lib.sh"

FIXTURE_DIR="$SCRIPT_DIR/fixtures/scenarios/ignore-feature/empty-ignore-file"
TEST_DIR="/tmp/stowaway-test-ignore-empty-file-$$"

echo "🧪 Testing .stowaway-ignore: empty ignore file..."

# Setup test environment
setup_test_env "$FIXTURE_DIR" "$TEST_DIR"

# Verify .stowaway-ignore file exists (but is empty)
verify_ignore_file_exists "$TEST_DIR" "Empty ignore file exists" || {
...
if [ "$PACKAGE_COUNT" -eq 2 ]; then
	echo "✅ Exactly 2 packages processed (empty file handled correctly)"
else
...
echo "🎉 Empty ignore file test completed successfully!"
