...
1. **Is it referenced?** Does the SKILL.md body mention operations requiring
   this tool?
2. **Is it necessary?** Could the skill achieve its purpose without this tool?
3. **Is the scope minimal?** Could a more restrictive tool achieve the same
   result?
...
- **Expected**: `Read, Grep, Glob, Write` or
...
- **If seen**: Requires strong justification — the skill should be doing
  something that genuinely needs broad access
