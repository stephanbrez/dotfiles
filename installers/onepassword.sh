...
install_onepassword() {
    ...
    if [[ "$pkgmgr" == "apt" ]]; then
        ...
        if should_run; then
            $ASME brew install --cask 1password
            $ASME brew install 1password-cli
            log_message "SUCCESS" "1Password installed via Homebrew"
        else
            dry_print "Would run: brew install --cask 1password && brew install 1password-cli"
        fi
    else
        log_message "WARNING" "1Password installation only configured for apt and brew" "true"
        return 1
    fi
}
